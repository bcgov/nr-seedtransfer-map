...
test.describe('CBST Seedlot Selection Tool - E2E Integration Tests', () => {
  ...
  test('Cutblock Flow: Selecting PLI species, IDFdk1 BEC, and 2053 year draws suitability regions on map', async ({
    ...
    // Verify the suitability layer was populated with BEC polygons from the
    // local FlatGeobuf snapshot (the layer's client-side source holds the
    // selected suitable variants' graphics).
    await expect
      .poll(
        async () => {
