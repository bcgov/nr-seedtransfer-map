...
define(['lib/flatgeobuf/flatgeobuf-geojson.min.js', 'scripts/dataUrl.js'], function (
  ...
  function loadBecBounds() {
    ...
    let boundsUrl = dataUrl.resolveDataUrl('Version_7_0/bec_bounds.json') + '?v=7.0.11'
  ...
  function mapInit() {
    ...
    map.on('load', function () {
      ...
      let mguUrl = dataUrl.resolveDataUrl('Version_7_0/Management_Units.fgb') + '?v=7.0.11'
      ;(async function () {
        try {
          const features = []
  ...
  function addFlatGeobufLayer(baseUrl, whereClause, layerId, fillColor, opacity) {
    ...
    loadBecBounds().then(function (bounds) {
      ...
      let targetUrl = dataUrl.resolveDataUrl(baseUrl) + '?v=7.0.11'
      ;(async function () {
        try {
